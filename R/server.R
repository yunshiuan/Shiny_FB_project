...
shinyServer(function(input, output) {
  ...
  output$Features <- renderPlotly(
    {
      if (input$Cluster == "2") {
        Cl <- ggplot(featurelong[which(featurelong$Cluster == "2"), ], aes(x = Issue, y = Zscore, group = 1)) +
          ...
          theme(
            axis.text.x = element_text(angle = 45, hjust = 1, size = axistext),
            axis.text.y = element_text(size = axistext),
            plot.title = element_text(face = "bold", size = maintitle),
            plot.subtitle = element_text(size = subtitle),
            axis.title.x = element_text(size = axistitle),
  ...
  output$AllCluFreq <- renderPlot({
    ggplot(featurelong, aes(x = Issue, y = Zscore, group = Cluster, color = Cluster)) +
      ...
      labs(
        y = "Z-score", title = "Tendency of all clusters", subtitle = "95% CI are shown by the bars.",
      ...
      scale_colour_manual(
        values =
          c("#a6cee3", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#cab2d6", "#ffff33", "#f781bf"), labels = c("Antinuclear", "EndPenalty", "No explicit opinion", "GayMarriage", "Homophobia", "(Extreme) Penalty", "Penalty", "Independence", "Reunification", "Nuclear")
      )
  })
  ...
  output$CluFreqno2 <- renderPlot({
    ggplot(featurelong[-which(featurelong$Cluster == "2"), ], aes(x = Issue, y = Zscore, group = Cluster, color = Cluster)) +
      ...
      geom_point(position = pd, size = 1) +
      labs(
        y = "Z-score", title = "Tendency of all clusters except for cluster 2",
      ...
      scale_colour_manual(
        values =
          c("#a6cee3", "#b2df8a", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#cab2d6", "#ffff33", "#f781bf"), labels = c("Antinuclear", "EndPenalty", "GayMarriage", "Homophobia", "(Extreme) Penalty", "Penalty", "Independence", "Reunification", "Nuclear")
      )
  })
  ...
  output$Cltable <- renderTable({
    ...
    if (input$FeatIssue == "FeatIssue3") {
      ...
      for (i in as.character(1:10)) {
        if (input$Cluster == i) {
          cltable <<- arrange(
            featurelong_ch[which(featurelong_ch$群集 == i), c(3, 7, 8)],
            desc(Z分數)
          )
        }
      }
  ...
  output$CluFreq <- renderPlotly(
    {
      if (!(input$Cluster == "All Clusters" || input$Cluster == "All Clusters (No Cluster 2)")) {
        df %>%
          ...
          select(matches(input$Issue)) %>%
          ggplot(aes_string(x = input$Issue)) + geom_histogram(aes()) +
          labs(
            x = paste(input$Issue, "Z-score"),
            title = paste0(
              "Cluster ", input$Cluster, " (N = ",
  ...
  output$CA3D_S <- renderPlotly({
    plot_ly(df_no2,
      x = ~EndPenalty, y = ~Independence, z = ~GayMarriage,
  ...
  output$CA3D_O <- renderPlotly({
    plot_ly(df_no2,
      x = ~Penalty, y = ~Reunification, z = ~Homophobia,
      color = ~cluster_chi, mode = "markers", marker = list(size = 3)
    ) %>%
      layout( # title = "支持",
        scene = list(
  ...
  output$dimchoice <- renderUI({
    if (input$dimchange == "支持") {
      tagList(
        selectInput(
          inputId = "dim_3DSOF_x", label = "x軸",
          choices = c(
            "廢死" = "EndPenalty", "臺獨" = "Independence", "同婚" = "GayMarriage",
      ...
      tagList(
        selectInput(
          inputId = "dim_3DTF_x", label = "x軸",
          choices = c(
            "死刑存廢" = "P_total", "臺灣統獨" = "T_total", "同婚與否" = "G_total",
            "反核有理" = "N_total"
          ),
          selected = "同婚與否"
        ),
        selectInput(
          inputId = "dim_3DTF_y", label = "y軸",
          choices = c(
            "死刑存廢" = "P_total", "臺灣統獨" = "T_total", "同婚與否" = "G_total",
            "反核有理" = "N_total"
          ),
  ...
  output$CA3D_SOF <- renderPlotly({
    plot_ly(df_no2,
      x = ~ eval(parse(text = input$dim_3DSOF_x)), y = ~ eval(parse(text = input$dim_3DSOF_y)), z = ~ eval(parse(text = input$dim_3DSOF_z)),
      color = ~cluster_chi, mode = "markers", marker = list(size = 3)
    ) %>%
      layout(
        scene = list(
  ...
  output$CA3D_TF <- renderPlotly({
    if (is.null(input$dim_3DTF_x) == F || is.null(input$dim_3DTF_y) == F || is.null(input$dim_3DTF_z) == F) {
      plot_ly(df_no2,
        x = ~ eval(parse(text = input$dim_3DTF_x)), y = ~ eval(parse(text = input$dim_3DTF_y)), z = ~ eval(parse(text = input$dim_3DTF_z)),
        color = ~cluster_chi, mode = "markers", marker = list(size = 3)
      ) %>%
        layout(
          scene = list(
  ...
  output$PotPlot <- renderPlot({
    ...
    ggplot(politics, aes(x = Issue, y = Zscore, group = Type, color = Type)) +
      ...
      geom_point(position = pd, size = 1) +
      labs(
        x = NULL, y = "Z-score", title = "Tendency of all clusters", subtitle = "95% CI are shown by the bars.",
      ...
      guides(size = F, alpha = F) +
      scale_colour_manual(
        values = c("#a6cee3", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#cab2d6", "#ffff33", "#f781bf")
  ...
  output$RelPlot <- renderPlot({
    ggplot(religion, aes(x = Issue, y = Zscore, group = Type, fill = Type)) +
      ...
      guides(size = F, alpha = F) +
      scale_fill_manual(
        name = "Cluster", values = c("#a6cee3", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#cab2d6", "#ffff33", "#f781bf")
        # , labels = c("Antinuclear", "EndPenalty","No explicit opinion", "GayMarriage", "Homophobia", "(Extreme) Penalty", "Penalty", "Independence", "Reunification", "Nuclear")
      ) +
  ...
  output$MedPlot <- renderPlot({
    ...
    ggplot(media, aes(x = Issue, y = Zscore, group = Type, color = Type)) +
      ...
      guides(size = F, alpha = F) +
      scale_colour_manual(
        values = c("#a6cee3", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#cab2d6", "#ffff33", "#f781bf", "#b2df8a")
