...
shinyUI(
  navbarPage(
    theme = shinytheme("simplex"), # 採用的模板
    "DSR Project", # 標題
      ...
      tabPanel(
        ...
        sidebarLayout(
          ...
          sidebarPanel(
            htmlOutput("CA3Dsidertext"),
            hr(),
            radioButtons("dimchange",
              label = "您想自由調整哪一種圖的維度？（結果請點選「自由繪製」的欄位）", c("支持", "立場加總"),
              inline = T
            ),
            uiOutput("dimchoice")
          ),
      ...
      tabPanel(
        ...
        sidebarLayout( # Generate a row with a sidebar
          sidebarPanel( # Define the sidebar with one input
            selectInput(
              inputId = "Cluster", label = "同溫層（在各議題的分數）",
              choices = c(
            ...
            selectInput(
              inputId = "Issue", label = "該同溫層在 _____ 立場的Z分數分配",
              choices = c(
                "廢死 (EndPenalty)" = "EndPenalty", "臺獨 (Independence)" = "Independence", "同婚 (GayMarriage)" = "GayMarriage",
